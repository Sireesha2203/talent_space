...
const verifySuperToken = async (req, res, next) => {
  ...
  userApp.post("/upload-file",multerObj.single('photo'),expressAsyncHandler(async(request,response)=>{
      //add CDN link of cloudinary image to user obj
      response.status(201).send({filePath:request.file.path})
...
userApp.get('/search-users/:userInput', verifyToken, expressAsyncHandler(async (req, res) => {
...
userApp.get('/set-default-password/:username',verifyToken,expressAsyncHandler(async(req,res)=>{
    try{
        //get user collection
        const userCollectionObj=req.app.get("userCollectionObj")
        ...
        let mailOptions = {
          ...
          to: userOfDB.email,
          subject: 'Password Changed by Super user - FIS',
...
//remove user
userApp.delete("/remove-user/:username",verifyToken,expressAsyncHandler(async(req,res)=>{
  try {
    // get user collection
    const userCollectionObj = req.app.get("userCollectionObj");
    ...
    let mailOptions = {
      ...
      text: 'Your access to the user page has been removed.',
    };
